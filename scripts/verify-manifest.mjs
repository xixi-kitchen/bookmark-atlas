...
if (!['chrome', 'edge', 'safari'].includes(browser)) throw new Error(`Unsupported browser target: ${browser}`);
const outputRoot = new URL(`../output/${browser}-mv3/`, import.meta.url);
...
if (JSON.stringify(permissions) !== JSON.stringify(expectedPermissions)) throw new Error(`Unexpected ${browser} permissions: ${permissions.join(', ')}`);
...
if (browser === 'safari') {
  if (manifest.minimum_chrome_version) throw new Error('Safari manifest must not contain minimum_chrome_version.');
  if (manifest.browser_specific_settings?.safari?.strict_min_version !== '17.0') {
    throw new Error('Safari manifest must declare strict_min_version 17.0.');
  }
}
...
console.log(`${browser} manifest verified: MV3, i18n locales, scoped permissions, remote suggestions, web embeds, icons present.`);

async function verifyLocale(locale) {
  const messages = JSON.parse(await readFile(new URL(`_locales/${locale}/messages.json`, outputRoot), 'utf8'));
